...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    ...
    fetchMyBlog(stored, id)
      .then((blog) => {
        setTitle(blog.title)
        setSubTitle(blog.subTitle || "")
        setAuthorName(blog.authorName || blog.author || "")
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await updateAuthorBlog(token, id, {
        title: title.trim(),
  ...
  if (loading) {
    return <div className="p-8 text-center text-slate-600">Loading blog...</div>
  }
      ...
      <main className="mx-auto max-w-2xl px-4 py-8 sm:py-10">
        ...
        {adminMessage && (
          <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3">
            <p className="text-xs font-semibold uppercase text-red-700">Admin feedback</p>
            <p className="mt-1 text-sm text-red-800">{adminMessage}</p>
            <p className="mt-1 text-xs text-red-600">Editing this blog will re-submit it for review.</p>
          </div>
        )}
